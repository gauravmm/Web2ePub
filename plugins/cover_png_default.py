...
class DefaultPNGCover(plugins.BaseCover):
	...
	def cover(self, bookmeta):
		im = Image.new("L", (600, 800), 0);
		draw = ImageDraw.Draw(im)
		
		# Background:
		draw.rectangle((10, 10, 590, 790), 0xFF);
		draw.rectangle((10, 10, 590, 400), 0x66);
		...
		offset = (floor(380/38) - len(tt))*20;
		f36 = ImageFont.truetype("verdana.ttf", 36) 
		for l in range(min(len(tt), MAX_CHAR_LINE)):
			sz = f36.getsize(tt[l]);
			draw.text(((600 - sz[0])/2, 40*l + offset), tt[l], fill=0xFF, font=f36);
			
		poly = [(74,91.57), (26,91.57), (2,50), (26,8.43), (74,8.43), (98,50), (74,91.57)];
		poly = [(x-50, y-50) for x, y in poly];
		draw.polygon([(300 + x*1.1, 400 + y*1.1) for x, y in poly], fill=0xFF);
		draw.polygon([(300 + x, 400 + y) for x, y in poly], fill=0x99);


		f20i = ImageFont.truetype("verdanai.ttf", 20);
		sz = f20i.getsize(bookmeta["author"]);
		draw.text(((600-sz[0])/2, 450), bookmeta["author"], fill=0x00, font=f20i);

		# Attrib
		if "attrib" in bookmeta:
			f24 = ImageFont.truetype("verdana.ttf", 24);
			sz = f24.getsize(bookmeta["attrib"]);
			draw.text(((600-sz[0])/2, 800 - 15 - sz[1]), bookmeta["attrib"], fill=0x99, font=f24);
